...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('kavachai.enterApiKey', async () => {
        ...
        if (apiKey) {
            ...
            try {
                const res = await (0, node_fetch_1.default)(`${BACKEND_URL}/api/apikey/validate`, {
                    method: 'POST',
                    headers: {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('kavachai.pushToGitHub', async () => {
        ...
        if (envOrGit.length > 0) {
            ...
            if (notIgnored.length > 0) {
                const action = await vscode.window.showWarningMessage(`🚨 CRITICAL SECURITY WARNING 🚨\n\nThe following sensitive files are NOT ignored and may be pushed to GitHub:\n\n${notIgnored.join(', ')}\n\nPushing these files can EXPOSE SECRETS or BREAK your security.\n\nWhat do you want to do?`, { modal: true }, 'Add to .gitignore', 'Cancel Push', 'Open .gitignore');
...
function showVulnerabilityPanel(result, context) {
    ...
    if (result.results) {
        ...
        result.results.forEach((fileResult) => {
            if (fileResult.issues) {
                fileResult.issues.forEach((issue) => {
                    ...
                    const isCommentOnly = issue.fixedCode && (issue.fixedCode.trim().startsWith('#') ||
        ...
        allVulnerabilities = result.vulnerabilities.map((vuln) => {
            const isCommentOnly = vuln.fixedCode && (vuln.fixedCode.trim().startsWith('#') ||
    ...
    const handledVulnerabilities = new Set();
    ...
    panel.webview.onDidReceiveMessage(async (message) => {
        ...
        switch (message.command) {
            ...
            case 'openFile':
                ...
                const currentVulns = getCurrentVulnerabilities();
                const vuln = currentVulns[message.vulnIndex] || currentVulns.find((v) => v.file === message.file && v.line === message.line);
                if (vuln) {
...
async function applyCodeFix(vulnerability) {
    try {
        ...
        if (isCommentOnly) {
            ...
            const action = await vscode.window.showWarningMessage(`This vulnerability requires manual intervention:\n\n${vulnerability.description}\n\nSuggestion: ${vulnerability.suggestion}\n\nWould you like to open the file to manually apply the fix?`, 'Open File', 'Cancel');
            if (action === 'Open File') {
        ...
        if (fixedCodeLines.length > 1) {
            ...
            const extendedRange = new vscode.Range(new vscode.Position(vulnerability.line - 1, 0), new vscode.Position(endLine, document.lineAt(endLine).text.length));
